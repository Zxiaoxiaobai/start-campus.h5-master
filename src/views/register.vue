<template>
    <div style="overflow: hidden" class="container">
        ...
        <div v-if="!data.isSendCode">
            <h1 class="title">嘿！ <br/>赶紧注册吧!</h1>
            <p class="sub-title">欢迎回来～ <br/>我们等你好久了!</p>
            <van-tabs v-model:active="data.active">
                <van-tab title="手动注册">
                    <div>
                        <van-form ref="manualRegisterFormRef">
                            <van-cell-group inset class="form-group">
                                <van-field v-model="manualRegisterForm.userNickName" placeholder="请输入昵称"
                                           class="form-field"/>
                                <van-field
                                    v-model="manualRegisterForm.password"
                                    type="password"
                                    name="password"
                                    placeholder="请输入密码"
                                    class="form-field"
                                />
                                <van-field
                                    v-model="manualRegisterForm.rePassword"
                                    type="password"
                                    name="rePassword"
                                    placeholder="请再次输入密码"
                                    class="form-field"
                            ...
                            <van-button type="primary" block round class="login-button"
                                        @click="submitRegister('manual')">立即注册
                            </van-button>
                ...
                <van-tab title="邮箱注册" :disabled="!data.isDevEnv">
                    <div>
                        <van-form ref="registerEmailFormRef">
                            <van-cell-group inset class="form-group">
                                <van-field v-model="registerForm.email" placeholder="请输入邮箱" class="form-field"/>
                            </van-cell-group>
                            <van-button type="primary" block round class="login-button" @click="sendCode">下一步
                            </van-button>
                            <div class="forget-password">
                                <span>忘记密码？</span>
                            </div>
            ...
            <p class="no-account" @click="toLogin">已有账号? 点击这里</p>
        </div>
        <!-- 输入验证码 -->
        <div v-if="data.isSendCode">
            <img :src="data.backImage" class="back-png" @click="data.isSendCode = false">
            <img :src="data.codeImage" class="frame-png">
            <p class="text">安全认证</p>
            <p class="code-text">我们已经发送6位数验证码至您的邮箱<br/>请输入验证码以证明是您本人</p>
            <div>
                <van-password-input
            ...
            <van-button type="success" class="confirm-code" size="large" @click="submitRegister('code')">确认</van-button>
        </div>
...
import {currentUserInfo, registerEmail, manualRegister} from "@/api/user/user";
import {setToken, setUserInfo} from '@/utils/auth'
import {Notify} from "vant";
import {useRouter} from 'vue-router'
import {sendEmailCode} from "../api/email/email";
import SendLoading from "@/components/send/sendLoading";
import {flowerDrops} from "@/assets/js/flowerDrops";
...
export default {
  created() {
    ...
    setup() {
        ...
        // 手动注册Form
        const manualRegisterForm = reactive({
            userNickName: '',
            password: '',
            rePassword: '',
        })
        const sendEmailCodeRo = reactive({
            email: ''
        })
        const data = reactive({
            active: 0,
            ...
            isSendCode: false,
            showKeyboard: false,
            isDevEnv: process.env.VUE_APP_CURRENTMODE === 'development'
        })
        ...
        const sendCode = async () => {
            if (registerForm.email === '') {
                Notify({type: 'danger', message: '请输入邮箱', duration: 1000});
                return;
            }
            const phonePattern = /^([a-zA-Z\d][\w-]{2,})@(\w{2,})\.([a-z]{2,})(\.[a-z]{2,})?$/;
            if (!phonePattern.test(registerForm.email)) {
                Notify({type: 'danger', message: '请输入正确的邮箱', duration: 1000});
                return;
            }
            ...
            sendShow.value = true
            sendEmailCodeRo.email = registerForm.email
            // 注册发送验证码
            sendEmailCode(sendEmailCodeRo).then(() => {
         ...
         * @returns {Promise<void>}
         */
        const submitRegister = (type) => {
            if (type === 'manual') {
                // 手动注册
                manualRegister(manualRegisterForm).then(res => {
                    setToken(res.data);
                    getCurrentUserInfo()
                })
            }
            if (type === 'email') {
                // 邮箱注册
                // 注册登录
                registerEmail(registerForm).then(res => {
                    setToken(res.data);
                    getCurrentUserInfo()
                })
            }
